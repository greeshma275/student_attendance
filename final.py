...
def login():
    ...
    st.image(r"C:\Users\Greeshma G\Pictures\Saved Pictures\image.jpg", width=900)
    ...
    if st.session_state.logged_in:
        ...
        if user_choice == "Staff":
            ...
            if student_choice == "Insert Student":
                ...
                address = st.text_input("New Address")
                branch = st.text_input("New Branch")
                b_id = st.number_input("New Branch id:", min_value=1, step=1)
            ...
            if admin_choice == "Insert Admin":
                ...
                a_id = st.number_input("Admin ID", min_value=1, step=1)
                a_name = st.text_input("Admin Name")
                if st.button("Insert Admin"):
                    query = "INSERT INTO ADMIN (A_ID, A_Name) VALUES (%s, %s)"
                    execute_query(query, (a_id, a_name))
                ...
                a_id = st.number_input("Admin ID to Update", min_value=1, step=1)
                a_name = st.text_input("New Admin Name")
                if st.button("Update Admin"):
                    query = "UPDATE ADMIN SET A_Name = %s WHERE A_ID = %s"
                    execute_query(query, (a_name, a_id))
                    st.success(f"Admin with ID {a_id} updated successfully.")

            elif admin_choice == "Delete Admin":
                st.subheader("Delete Admin Data")

                a_id = st.number_input("Admin ID to Delete", min_value=1, step=1)
                if st.button("Delete Admin"):
                    query = "DELETE FROM ADMIN WHERE A_ID = %s"
                    execute_query(query, (a_id,))
                    st.success(f"Admin with ID {a_id} deleted successfully.")
            ...
            if branch_choice == "Insert Branch":
                ...
                if st.button("Insert Branch"):
                    query = "INSERT INTO BRANCH (B_ID, B_Name, Seats, A_ID) VALUES (%s, %s, %s, %s)"
                    execute_query(query, (b_id, b_name, seats, a_id))
                ...
                a_id = st.number_input("New Admin ID", min_value=1, step=1)
                if st.button("Update Branch"):
                    query = "UPDATE BRANCH SET B_Name = %s, Seats = %s, A_ID = %s WHERE B_ID = %s"
                    execute_query(query, (b_name, seats, a_id, b_id))
            ...
            if attendance_choice == "Insert Attendance":
                ...
                if st.button("Update Attendance"):
                    ...
                    st.success(f"Attendance for Enroll No {enroll_no} updated successfully.")
                ...
                if st.button("Delete Attendance"):
                    ...
                    execute_query(query, (enroll_no,))
                    st.success(f"Attendance for Enroll No {enroll_no} deleted successfully.")
            ...
            if absent_choice == "Insert Absent":
                ...
                if st.button("Update Absent"):
                    ...
                    st.success(f"Absent data for Enroll No {enroll_no} updated successfully.")
                ...
                if st.button("Delete Absent"):
                    ...
                    execute_query(query, (enroll_no,))
                    st.success(f"Absent data for Enroll No {enroll_no} deleted successfully.")
